...
public class KSP {
	...
	public HashMap<HashMap<String,Integer>,Double> 
		...
		LinkedList<Entry<String, Point>> rootList = recursiveFunc(
				queryKeywords.size()-1, queryKeywords);
		...
		double spatialDistance = distanceQd + (validGroup.size()-1)*distancePd;
		
		double keywordRelevance = calculateKeywordRelevance(
		...
		System.out.println("KSP elapsed time is: "
				+ bckExecutionTime + " ms.");
		System.out.println("size of RSP is: "
				+ sizeOfRSP);
	...
	public HashMap<Integer,LinkedList<String>> 
		selectSubkeywords(int r, LinkedList<String> querykeywords)
    {
	...
	public LinkedList<Entry<String, Point>> recursiveFunc(int numOfIter, LinkedList<String> queryKeywords) {
		LinkedList<Entry<String, Point>> result = new LinkedList<Entry<String, Point>>();
		...
		HashMap<Integer, LinkedList<String>>
			partOfKeywordsHash = selectSubkeywords(numOfIter, queryKeywords);
		HashMap<Entry<String, Point>, LinkedList<String>> 
		rootUncoverdKeywords = findSP(partOfKeywordsHash, queryKeywords);
		
		if(rootUncoverdKeywords.isEmpty()) {
			result = recursiveFunc(numOfIter-1, queryKeywords);
		}
		
		else
		{
			for(java.util.Map.Entry<Entry<String, Point>, LinkedList<String>> entry : rootUncoverdKeywords.entrySet()) {
			    ...
			    if(value.isEmpty())
			    {
			    	...
			    	result.addAll(recursiveFunc(value.size(), value));
			    }
			}	
			return result;
		}
